...
1. **Enforceable hard deadline** — effective date, active enforcement, and material penalties
2. **Effort-to-impact ratio** — policy language update is cheap; product rebuild is not
3. **Existing partial compliance** — if GDPR compliance already covers most of a requirement, the state-law delta may be small
...
>
> - `[settled — last confirmed YYYY-MM-DD]` — stable, well-known statutory and regulatory references that have been checked against a primary source on the stated date (e.g., GDPR Art. 33, CCPA § 1798.100, FTC Act § 5). The date matters — even "stable" references change. When you can't confirm the date of the last check, use `[model knowledge — verify]` instead; an unconfirmed "settled" is a confident overclaim. Still verify before filing, but lower priority.
