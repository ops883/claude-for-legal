...
The practice-level CLAUDE.md has a `Cross-matter context:` flag. When it's `off` (the default), a skill working in matter A **never reads** files in `matters/B/` for any other `B`, without exception. This is the confidentiality guarantee the setting exists to provide.
